#modified By J.Tang from lume-genesis version 4. 
...
#from .lattice import parse_genesis4_lattice_file, taper, write_linear_taper_sec,write_undulator
...
class Genesis4(CommandWrapper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.input_file:
            ...
            self.init_lattice_filename = self.lattice_filename = self.input['main'][0]['lattice']
            #self.init_lattice = self.lattice = parse_genesis4_lattice_file(self.init_lattice_filename)
        self.configure()
    ...
    def make_new_lattice(self, undKs, und_period, und_nperiods,  fodo_length, 
                         ...
                         latticefilepath = None, linename = 'myline', tag = '',
                         apply_taper = False, dKbyK = 0.0, ustart = 0, ustop = 0, order = 2):
        """
        create a new lattice with arbitrary K distribution
        """
        if not latticefilepath:
            latticefilepath = self.path +'/' + self.input["main"][0]["rootname"] + '.lat'
        output_lines = make_lattice(undKs = undKs, und_period = und_period, und_nperiods = und_nperiods,  fodo_length = fodo_length, 
                                    quad_length = quad_length, quad_grad = quad_grad, 
                                    phaseShifts = phaseShifts, phaseshift_length = phaseshift_length, 
                                    use_chicane = use_chicane, chicane_pos = chicane_pos, l = l, lb = lb, ld = ld, delay = delay,
                                    latticefilepath = latticefilepath, linename = linename, tag = tag,
                 apply_taper = apply_taper, dKbyK = dKbyK, ustart = ustart, ustop = ustop, order = order, append = False)
        self.lattice = output_lines
        self.lattice_filename =  latticefilepath
        self.linename = linename
        self.input['main'][0]['lattice'] = self.lattice_filename
        self.input['main'][0]['beamline'] = linename

    
    def append_new_line(self, undKs, und_period, und_nperiods,  fodo_length, 
                         quad_length, quad_grad, 
                         phaseShifts = None, phaseshift_length = 0.0, 
                         use_chicane = False, chicane_pos = 8, l = 3.4, lb = 0.1, ld = 1.2, delay = 15e-15*3e8,
                        linename = 'myline', tag = 'a',
                         apply_taper = False, dKbyK = 0.0, ustart = 0, ustop = 0, order = 2):
        ...
        output_lines = make_lattice(undKs = undKs, und_period = und_period, und_nperiods = und_nperiods,  fodo_length = fodo_length, 
                                    ...
                                    phaseShifts = phaseShifts, phaseshift_length = phaseshift_length, 
                                    use_chicane = use_chicane, chicane_pos = chicane_pos, l = l, lb = lb, ld = ld, delay = delay,
                                    latticefilepath = self.lattice_filename, linename = linename, tag = tag,
                 apply_taper = apply_taper, dKbyK = dKbyK, ustart = ustart, ustop = ustop, order = order, append = True)
        self.lattice2 = output_lines
        self.linename2 = linename
        
    
        
    
    #def apply_taper(self, write_filename, Kstart, dKbyK, ustart, ustop, nwig, uperiod, order = 2):
    #    """
    #    read from an existing lattice file and modifiying undulator
    #    """
        
    #    output_lines = []
    #    for key in self.init_lattice:
    #        if key != 'Undulator' and key != 'Line':
    #            output_lines.extend(self.init_lattice[key])

    #    usegname_list = []
    #    for und in self.init_lattice['Undulator']:
    #        usegname_list.append(und.split(':')[0])    
    #    lines = write_undulator(usegname_list, Kstart, dKbyK, ustart, ustop, nwig, uperiod, order)
    #    output_lines.extend(lines)
    #    
    #    output_lines.extend(self.init_lattice['Line'])
    #    
    #    self.lattice = output_lines
    #    self.lattice_filename = write_filename
    #    with open(self.lattice_filename,'w') as tfile:
    #    	tfile.write(''.join(output_lines))
    #    self.input['main'][0]['lattice'] = self.lattice_filename

    #def get_batch_run_script(runscript_path = "/sdf/group/ad/beamphysics/jytang/lume-genesis/RunGenesis.sh"):
    #    shutil.copyfile(run_script_path, self.path + "/RunGenesis.sh")
    
    

    def batch_run(self, runscript = "cavity_genesis4/genesis4/lume_genesis_JT/RunGenesis.sh"):
        ...
        #runscript = self.get_run_script(infile = self.input["main"][0]["rootname"] + '.in')
        #shutil.copyfile(runscript, self.path + "/RunGenesis.sh")
        
        inputfile = self.input["main"][0]["rootname"] + '.in'
        #runscript = "salloc --partition milano --account ad:beamphysics --mem-per-cpu=4g  -n {nproc} /usr/lib64/openmpi/bin/mpirun -n {nproc} {command_mpi} {inputfile}".format(nproc = self.nproc, command_mpi = self.command_mpi, inputfile = inputfile)
        ...
        # Write input
        self.write_input(input_filename=self.input["main"][0]["rootname"] + '.in')

        cwd = os.getcwd()
        os.system('cp  ' + runscript + ' ' + self.path)
        os.chdir(self.path)
        self.jobid = submit_genesis(input_file=inputfile, shell_script = 'RunGenesis.sh')
        os.chdir(cwd)

    def wait_and_process(self, archive = None):
        ...
        if archive:
            #self.load_output()
            savedir = archive #archive + '/' + self.input["main"][0]["rootname"]
            #if os.path.exists(savedir):
            #    shutil.rmtree(savedir)
            #shutil.copytree(self.path, savedir)
            #archivename =  savedir +  '/' + self.input["main"][0]["rootname"] + '.h5'
            #self.archive(archivename)
            files=os.listdir(self.path)
            for fname in files:
                shutil.copy2(os.path.join(self.path,fname), savedir)
            self.finished = True
    ...
    def get_run_script(self, write_to_path=False, path=None, scriptname="run", infile = "genesis4.in"):
        """
    ...
    def write_input(self, path=None, input_filename="genesis4.in"):
        ...
        # Write run script
        #self.get_run_script(write_to_path=True, path=path, infile = input_filename)
