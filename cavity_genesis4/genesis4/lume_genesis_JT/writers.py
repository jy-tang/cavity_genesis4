...
def write_dfl_genesis4_h5(fld, param, filename, indexing = 'Genesis2'):
    #J. Tang 3/25/2024
    if indexing == 'Genesis2':
        print('Assuming fld in dimension [t, x, y], writing field to h5 file ', filename)
        fld = np.moveaxis(fld, 0, -1)
    else:
        print('Assuming fld in dimension [x, y, t], writing field to h5 file ', filename)
    ...
    with h5py.File(filename, "w") as f:
        ...
        for i in range(fld.shape[2]):
            ...
            g.create_dataset('field-real', data = np.real(fld[:,:, i].flatten()))
            g.create_dataset('field-imag', data = np.imag(fld[:,:, i].flatten()))